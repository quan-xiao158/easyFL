...
def init(task: str, algorithm, option = {}, model=None, Logger: flgo.experiment.logger.BasicLogger = None, Simulator: BasicSimulator=flgo.simulator.DefaultSimulator, scene='horizontal'):
    ...
    # init option
    option = load_configuration(option)#读cpu还是gpu 读训练轮次，option是字典  num—_round=？、num_epoch=?、cpu=0
    default_option = read_option_from_command()#从命令行读配置信息
    for op_key in option:
        if op_key in default_option.keys(): #如果传进来的参数不等于默认的参数，那么就进行替换
            ...
            default_option[op_key] = option[op_key]
    option = default_option#将默认参数复制给option
    setup_seed(seed=option['seed'])#设置随机种子
    option['task'] = task#任务路径
    option['algorithm'] = (algorithm.__name__).split('.')[-1] #算法名称 fedavg
    option['scene'] = scene#场景设置
    ...
    with open(os.path.join(task, 'info'), 'r') as inf:
        task_info = json.load(inf)#载入路径下的json任务信息
    # benchmark information
    benchmark = task_info['benchmark']#info文件下的信息
    if 'bmk_path' in task_info and os.path.isdir(task_info['bmk_path']): sys.path.append(task_info['bmk_path'])
    # model information
    if model== None:
        bmk_module = importlib.import_module(benchmark)#没指定模型，bmk_module根据info文件里面的信息载入模型
        if hasattr(algorithm, 'init_global_module') or hasattr(algorithm, 'init_local_module'):#如果算法参数里面制定了模型，
            model = algorithm
        elif hasattr(bmk_module, 'default_model'):
            model = getattr(bmk_module, 'default_model')#返回bmk_module对象的参数default_model对应的模型
        else:
    ...
    option['model'] = (model.__name__).split('.')[-1]#将模型放到option里
    ...
    # init device#初始化设备，判断是使用cpu还是gpu
    ...
    if scene in ['horizontal', 'vertical', 'decentralized', 'hierarchical','parallel_horizontal']:
        ...
        # init objects
        obj_class = [c for c in dir(algorithm) if not c.startswith('__')]#初始化一个server一个client
        tmp = []#用来保存客户端和服务器端
        for c in obj_class:
            try:
        ...
        objects = task_pipe.generate_objects(option, algorithm, scene=scene)#100个元素的list
    ...
    ############################################### Real World FL #####################################################
        ...
        gv.communicator = flgo.VirtualCommunicator(objects)
        for ob in objects: ob.initialize()
    return objects[0]#第0个是服务器，剩下是客户端
