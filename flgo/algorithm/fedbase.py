...
class BasicServer(BasicParty):
    ...
    def iterate(self):
        ...
        self.selected_clients = self.sample()  # 选择客户端
        # training
        models = self.communicate(self.selected_clients)['model']  # 拿到选择的客户端进行训练并返回客户端的模型
        # aggregate: pk = 1/K as default where K=len(selected_clients)
        self.model = self.aggregate(models)  # 对模型进行聚合
        return len(models) > 0
