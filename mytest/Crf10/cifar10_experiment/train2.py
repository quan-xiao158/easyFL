...
if __name__ == '__main__':
    ...
    task1 = flgo.init(task=task, algorithm=fedasync,Logger=logger, option={'num_rounds': 3000, 'num_epochs': 20, 'gpu': 0,'b':0.7,'t':40,"plot":True,"client_weight":"uniform"})
    task1.run()
    task2 = flgo.init(task=task, algorithm=fedasync, Logger=logger,option={'num_rounds': 3000, 'num_epochs': 20, 'gpu': 0, 'b': 0.7, 't': 100, "plot": True,"client_weight": "uniform"})
    task2.run()