...
#CNN-KAFL
    ...
    task7.run()
#CNN-Fedbalance
    task1 = flgo.init(task=task,Logger=logger, algorithm=fedbalance, option={'num_rounds': 4000, 'num_epochs': 20, 'gpu': 0,'proportion':0.2,'b':0.3,'t':5,"client_weight":"random55",'plot':True},model=cnn)
    task1.run()
    task2 = flgo.init(task=task,Logger=logger, algorithm=fedbalance, option={'num_rounds': 4000, 'num_epochs': 20, 'gpu': 0,'proportion':0.2,'b':0.3,'t':40,"client_weight":"random55",'plot':True},model=cnn)
    task2.run()
    task3 = flgo.init(task=task, Logger=logger, algorithm=fedbalance,option={'num_rounds': 4000, 'num_epochs': 20, 'gpu': 0, 'proportion': 0.2, 'b': 0.3, 't': 100,"client_weight": "random55", 'plot': True}, model=cnn)
    task3.run()
    task5 = flgo.init(task=task,Logger=logger, algorithm=fedbalance, option={'num_rounds': 4000, 'num_epochs': 20, 'gpu': 0,'proportion':0.2,'b':0.7,'t':5,"client_weight":"random55",'plot':True},model=cnn)
    task5.run()
    task6 = flgo.init(task=task, Logger=logger, algorithm=fedbalance,option={'num_rounds': 4000, 'num_epochs': 20, 'gpu': 0, 'proportion': 0.2, 'b': 0.7, 't': 40,"client_weight": "random55", 'plot': True}, model=cnn)
    task6.run()
    task7 = flgo.init(task=task,Logger=logger, algorithm=fedbalance, option={'num_rounds': 4000, 'num_epochs': 20, 'gpu': 0,'proportion':0.2,'b':0.7,'t':100,"client_weight":"random55",'plot':True},model=cnn)
    task7.run()
