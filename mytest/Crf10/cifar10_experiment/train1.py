...
#3500轮后现存不够
    task1 = flgo.init(task=task, algorithm=fedasync,Logger=logger, option={'num_rounds': 3000, 'num_epochs': 20, 'gpu': 0,'b':0.3,'t':100,"plot":True,"client_weight":"uniform"})
    task1.run()
    task2 = flgo.init(task=task, algorithm=fedasync, Logger=logger,option={'num_rounds': 3000, 'num_epochs': 20, 'gpu': 0, 'b': 0.7, 't': 10, "plot": True,"client_weight": "uniform"})
    task2.run()
