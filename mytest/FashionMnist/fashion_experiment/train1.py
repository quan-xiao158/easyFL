...
if __name__ == '__main__':
    #已完成
    task1 = flgo.init(task=task, algorithm=fedbuff,Logger=logger, option={'num_rounds': 1000, 'num_epochs': 20, 'gpu': 0,'b':0.5,'t':20,"plot":True,"client_weight":"uniform"})
    task1.run()
    task1 = flgo.init(task=task, algorithm=fedbuff,Logger=logger, option={'num_rounds': 1000, 'num_epochs': 20, 'gpu': 0,'b':0.5,'t':25,"plot":True,"client_weight":"uniform"})
    task1.run()
