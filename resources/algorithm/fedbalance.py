...
class Server(AsyncServer):
    ...
    def fedbalance_late_aggregate(self, models, client_ids):
        total_data_vol = sum([self.clients[client_id].datavol for client_id in client_ids])#所有数据量
        avg_data_total=(total_data_vol/100)*self.current_round#平均数据量
        Act_data_total=[]
        train_list=[[]]
        sum_t=[]
        for id in  client_ids:
            Act_data_total[id]=self.clients[id].datavol*self.commit_num[id] #实际贡献量
            train_list[id]=models[id]._train_list
            sum_t[id]=sum(train_list)
            alpha_ts= [self.option['alpha']] * min(1,(sum_t[id]*(avg_data_total-Act_data_total[id]))/sum_t)
...
class Client(BasicClient):
    ...
    def model_train(self, model):
        ...
        for iter in range(10):
            ...
            loss1=loss+self.calculate_emd(self.model,src_model)
            loss2 = loss + self.calculate_emd(src_model, self.model)
            loss1.backward()
            loss2.backward()
    ...
    def calculate_emd(self,model1, model2):
        """
        计算两个PyTorch模型参数之间的EMD距离
        """
        emd_distances = []
        ...
        for (name1, param1), (name2, param2) in zip(model1.named_parameters(), model2.named_parameters()):
            ...
            # 展平参数
            flat_param1 = param1.detach().cpu().flatten().numpy()
            flat_param2 = param2.detach().cpu().flatten().numpy()

            # 计算EMD/Wasserstein距离
            emd = wasserstein_distance(flat_param1, flat_param2)
            emd_distances.append(emd)

        # 返回各层EMD的平均值
        return sum(emd_distances) / len(emd_distances)