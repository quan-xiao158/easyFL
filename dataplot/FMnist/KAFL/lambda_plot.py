...
def plot_data(record_list, line_names, title=None):
    ...
    """
    plt.rcParams.update({'font.size': 17})  # 全局字体大小设置
    ...
    if title:
        # 关键修改点3：调整y值和垂直对齐方式
        plt.suptitle(title, fontsize=17, fontname='SimSun',
    ...
    ax.set_ylabel('Average Obsolescence', fontname='Times New Roman')
    ax.grid(True)
    ...
    plt.savefig("./photo/lambda_KAFL_plot.png", bbox_inches='tight', dpi=300)  # 确保保存为PNG
    plt.show()

if __name__ == "__main__":
    folder_path = './lambda'  # 文件夹路径

    # 指定要读取的文件名（不含扩展名），顺序自定义
    custom_filenames = ["KAFL_0.3b_10t", "KAFL_0.3b_100t", "KAFL_0.7b_10t", "KAFL_0.7b_100t"]

    # 加载数据
    record_list, line_names = load_json_data_by_filenames(custom_filenames, folder_path)

    # 绘制图表
    plot_data(record_list, line_names, title="(a) KAFL平均陈旧度")