...
def plot_data(record_list, line_names, x_ticks,title):
    """绘制折线图"""
    plt.rcParams.update({'font.size': 15})  # 全局字体大小设置
    ...
    linestyles = ['-.']  # 定义线型列表
    if title:
        plt.suptitle(title, fontsize=15, fontname='SimSun',
    ...
    ax.legend(fontsize=15)  # 与 prop={'size':16} 等效
    # ax.legend()
    ...
    plt.savefig("./photo/cifar10",  bbox_inches='tight',dpi=300)
    plt.show()

if __name__ == "__main__":
    folder_path = './'  # 文件夹路径
    line_names = ["FedQS+FedTAT+FedKT", "FedQS+ FedKT", "FedQS+FedTAT", "FedTAT+ FedKT"]
    ...
    # 绘制图表
    plot_data(averaged_record_list, line_names, x_ticks,title="(c) CIFAR-100")