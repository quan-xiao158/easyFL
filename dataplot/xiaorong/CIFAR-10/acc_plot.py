...
def average_every_100(data):
    ...
    for series in data:
        # 将数据分成每100个一组
        groups = [series[i:i + 15] for i in range(0, len(series), 15)]
        # 对每组求平均值
        averaged_series = [np.mean(group) for group in groups]
        averaged_data.append(averaged_series)
        # 生成对应的横坐标（每组的起始点的索引）
        x_ticks = [i * 15 for i in range(len(groups))]  # 每组的起始round数
...
if __name__ == "__main__":
    ...
    # 绘制图表
    plot_data(averaged_record_list, line_names, x_ticks,title="(b) CIFAR-10")