...
def plot_data(record_list, line_names, x_ticks,title):
    ...
    plt.savefig("./photo/acc_3b100t_plot", dpi=600, bbox_inches='tight')
    plt.show()

if __name__ == "__main__":
    folder_path = './'  # 文件夹路径
    # custom_filenames = ["ρ=0.25_β=0.6", "ρ=0.25_β=0.7", "ρ=0.25_β=0.8", "ρ=0.1_β=0.75",
    #                     "ρ=0.2_β=0.75", "ρ=0.3_β=0.75"]
    custom_filenames = ["fedbuff_0.5b_10t", "fedbuff_0.5b_15t", "fedbuff_0.5b_20t", "fedbuff_0.5b_25t","fedbuff_0.5b_30t","fedbuff_0.5b_35t"]
    # 加载数据
    record_list = load_json_data(folder_path,custom_filenames)
    # 每100个点求平均值，并获取对应的横坐标
    averaged_record_list, x_ticks = average_every_100(record_list)
    # 绘制图表
    plot_data(averaged_record_list, custom_filenames, x_ticks,title="Accuracy of different superparameters of the algorithm on CIFAR-10")