...
def plot_data(record_list, line_names, title=None):
    ...
    plt.savefig("./photo/acc_7b10t_plot", dpi=600, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    # custom_filenames = ["ρ=0.25_β=0.6", "ρ=0.25_β=0.7", "ρ=0.25_β=0.8", "ρ=0.1_β=0.75","ρ=0.2_β=0.75","ρ=0.3_β=0.75"]
    custom_filenames = ["lr_fedbuff_0.5b_10t", "lr_fedbuff_0.5b_15t", "lr_fedbuff_0.5b_20t", "lr_fedbuff_0.5b_25t","lr_fedbuff_0.5b_30t","lr_fedbuff_0.5b_35t"]
    # 加载数据
    record_list, line_names = load_json_data_by_filenames(custom_filenames, folder_path)

    # 绘制图表
    plot_data(record_list, line_names, title="Accuracy of different superparameters of the algorithm on Fashion-MNIST")